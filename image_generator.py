...
# -*- coding: utf-8 -*-
"""
Генератор изображений с курсами валют - ИДЕНТИЧНАЯ копия
"""
...
class CurrencyImageGenerator:
    """Генератор изображений с курсами валют - точная копия"""
    ...
    def generate_currency_card(self, buy_rate: float, sell_rate: float, avg_rate: float) -> Optional[str]:
        """
        Генерирует изображение с курсами валют ИДЕНТИЧНО как на скрине
    ...
    def _generate_with_pillow(self, buy_rate: float, sell_rate: float, avg_rate: float) -> Optional[str]:
        """Генерация через Pillow - ИДЕНТИЧНАЯ копия"""
        ...
        # Добавляем DX паттерн - точно как на скрине
        ...
        # Размеры шрифтов точно как на скрине
        title_font = None
        rate_font = None
        time_font = None
        ...
        for font_path in font_paths:
            if os.path.exists(font_path):
                try:
                    title_font = ImageFont.truetype(font_path, 24)  # Маленький заголовок
        ...
        # Основная карточка - точно как на скрине
        card_width = 1200
        card_height = 400
        ...
        # Заголовок "USDT/RUB" - маленький, вверху
        title_text = "USDT/RUB"
        ...
        line_y = card_y + 80
        draw.line([(card_x + 60, line_y), (card_x + card_width - 60, line_y)], fill='#e5e7eb', width=1)
        
        # Курсы - точно как на скрине
        y_offset = line_y + 40
        ...
        # Карточка времени - точно как на скрине
        time_card_width = 400
        time_card_height = 60
    ...
    def _add_dx_pattern(self, draw):
        """Добавляет DX паттерн - точно как на скрине"""
